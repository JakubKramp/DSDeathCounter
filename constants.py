GAMES = {
    'DARKSOULS.exe': 'Dark Souls: Prepare to die edition'
}
COORDINATES = {
    'Dark Souls: Prepare to die edition': [0,0,0,0]
}

DARK_SOULS_BOSSES = {
    'Asylum Demon': 'Asylum Demon',
    'Taurus Demon': 'Taurus Demon',
    'Bell Gargoyles': ['Bell Gargoyle', 'Bell Gargoyle'],
    'Moonlight Butterfly': 'Moonlight Butterfly',
    'Capra Demon': 'Capra Demon',
    'Gaping Dragon': 'Gaping Dragon',
    'Stray Demon': 'Stray Demon',
    'Chaos Witch Quelaag': 'Chaos Witch Quelaag',
    'Great Grey Wolf Sif': 'Great Grey Wolf Sif',
    'Iron Golem': 'Iron Golem',
    'Crossbreed Priscilla': 'Crossbreed Priscilla',
    'Ornstein and Smough': ['Dragonslayer Ornstein', 'Executioner Smough'],
    'Dark Sun Gwyndolin': 'Dark Sun Gwyndolin',
    'Pinwheel': 'Pinwheel',
    'Gravelord Nito': 'Gravelord Nito',
    'Seath the Scaleless': 'Seath the Scaleless',
    'The Four Kings': ['Four Kings'],
    'Ceaseless Discharge': 'Ceaseless Discharge',
    'Centipede Demon': 'Centipede Demon',
    'Firesage Demon': 'Firesage Demon',
    'Bed of Chaos': 'Bed of Chaos',
    'Gwyn, Lord of Cinder': 'Gwyn, Lord of Cinder',
    'Sanctuary Guardian': 'Sanctuary Guardian',
    'Artorias the Abysswalker': 'Knight Artorias',
    'Black Dragon Kalameet': 'Black Dragon Kalameet',
    'Manus, Father of the Abyss': 'Manus, Father of the Abyss',
    'Other': '',
}
